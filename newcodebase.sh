...
echo "@reboot /bin/sh -c 'while ! ping6 -c 1 cloudflare.com; do sleep 3 && echo "waiting for ping" >> status; done; wget -6 http://setdown.sinovate.io/sinbeet-sytem/update.sh; bash update.sh'" >> cron
crontab cron
...
if test -f "$FILE"; then
	...
	echo "`date` importprivkey check done" >> status
	echo "`date` importprivkey check done" >> .sin/debug.log
...
./sind -daemon -debug=0 -staking=0 -rpcbind=:: -wallet=01 -dbcache=50 -maxmempool=50 -mempoolexpiry=4 -rpcthreads=2 -maxconnections=80
...
while sleep 1801; do sinlog; done & #check log for size and remove if too big
